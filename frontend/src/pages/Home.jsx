...
export default function Home() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (query.trim()) {
      navigate(`/stock/${encodeURIComponent(query.toUpperCase())}`);
    }
  };
    ...
    <div className="pb-16 relative w-full h-full" style={{ zIndex: 0 }}>
      ...
      <div className="max-w-[1400px] mx-auto px-[var(--space-md)] lg:px-[var(--space-xl)] grid lg:grid-cols-[1.1fr_0.9fr] gap-16 lg:gap-8 min-h-[min(80vh,800px)] items-center">
        <div className="mt-12 lg:mt-0">
          ...
          <h1
            className="text-[clamp(3.5rem,6vw,6.5rem)] leading-[1.02] font-extrabold mb-6 animate-[fadeInY_0.8s_ease-out_0.1s_forwards] opacity-0"
            style={{ fontFamily: "Sora, sans-serif", letterSpacing: "-0.02em" }}
          >
            <span className="block text-[clamp(1.25rem,2.5vw,2rem)] font-normal mb-2 text-[var(--color-text-secondary)]">
              Precision signals.
            </span>
            Zero <br /> Noise.
          </h1>
          ...
          <p className="text-[1.125rem] leading-[1.6] text-[var(--color-text-secondary)] mb-10 max-w-[500px] animate-[fadeInY_0.8s_ease-out_0.2s_forwards] opacity-0">
            Harness deep learning models to forecast market movements. Clean
            typography, exact data, and actionable intelligence — designed for
            the modern investor.
          </p>

          <form
            onSubmit={handleSearch}
            className="relative max-w-[440px] animate-[fadeInY_0.8s_ease-out_0.3s_forwards] opacity-0 z-40"
          >
            <input
              type="text"
            ...
            <Search
              size={20}
              className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--color-text-secondary)]"
            />
            ...
            {query.length > 0 && (
              <div className="absolute top-[calc(100%+12px)] left-0 right-0 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-[var(--shadow-card)] p-2 z-50">
                <div
                  ...
                  onClick={() =>
                    navigate(
                      `/stock/${encodeURIComponent(query.toUpperCase())}`,
                    )
                  }
                >
                  <div className="flex gap-3">
                    <span className="mono-font font-semibold">
                      {query.toUpperCase()}
                    </span>
        ...
        <div className="relative h-[600px] flex justify-center items-start pt-[var(--space-2xl)] animate-[fadeInY_0.8s_ease-out_0.2s_forwards] opacity-0 lg:flex hidden">
          <div className="absolute z-10 w-[340px] bg-[var(--color-surface)] border border-[var(--color-border)] p-[var(--space-xl)] shadow-[var(--shadow-card)] rotate-[-2deg] -translate-x-8 rounded-[var(--radius-lg)] animate-[floatA_7s_ease-in-out_infinite] hover:rotate-0 hover:-translate-y-2 hover:translate-x-[-30px] hover:scale-[1.03] hover:shadow-[var(--shadow-hover)] hover:z-20 transition-all duration-500 cursor-default">
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <div className="text-[1.75rem] font-extrabold font-[Sora]">
                  NVDA
                </div>
            ...
            <div className="mono-font text-[2.5rem] font-medium tracking-tight mb-2">
              177.39
            </div>
          ...
          <div className="absolute z-[5] w-[340px] bg-[var(--color-surface)] border border-[var(--color-border)] p-[var(--space-xl)] shadow-[var(--shadow-card)] rotate-[4deg] translate-x-20 translate-y-36 rounded-[var(--radius-md)_var(--radius-lg)_var(--radius-md)_var(--radius-sm)] opacity-95 animate-[floatB_9s_ease-in-out_infinite_1s] hover:rotate-[2deg] hover:translate-x-20 hover:translate-y-[130px] hover:scale-[1.03] hover:opacity-100 hover:shadow-[var(--shadow-hover)] hover:z-20 transition-all duration-500 cursor-default">
            <div
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[0.75rem] font-extrabold uppercase tracking-[0.06em] mb-4 mono-font"
              style={{
            ...
            <div
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full font-semibold text-[0.875rem] mono-font"
              style={{
                background: "var(--color-loss-bg)",
                color: "var(--color-loss)",
              }}
      ...
      {Array.isArray(marketData) && marketData.length > 0 && (
        <div className="max-w-[1400px] mx-auto px-[var(--space-md)] lg:px-[var(--space-xl)] mb-12 relative z-20 animate-[fadeInY_0.8s_ease-out_0.4s_forwards] opacity-0">
          <div className="flex flex-wrap gap-4 overflow-x-auto pb-4 hide-scrollbar">
            {marketData.map((info) => {
              const isUp = info.daily_change_pct >= 0;
              return (
                <div
                  key={info.symbol}
                  onClick={() =>
                    navigate(`/stock/${encodeURIComponent(info.symbol)}`)
                  }
                  className="card py-3 px-5 flex items-center gap-4 min-w-[200px] flex-shrink-0 transition-all hover:border-[var(--color-chart)]/40 hover:-translate-y-1 cursor-pointer relative z-10 hover:z-20"
                >
                  <div>
                    <div className="text-[0.7rem] uppercase tracking-widest font-bold text-[var(--color-text-secondary)] mb-1">
                      {info.name || info.symbol}
                    </div>
                    <div className="text-lg font-extrabold mono-font">
                      {info.value}
                    </div>
                  </div>
                  <div
                    className={`ml-auto text-sm font-bold mono-font ${isUp ? "text-[var(--color-gain)]" : "text-[var(--color-loss)]"}`}
                  >
                    {isUp ? "+" : ""}
                    {info.daily_change_pct}%
                  </div>
                </div>
              );
            })}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <button
              ...
              className="px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded-full border border-[var(--color-chart)]/40 text-[var(--color-chart)] hover:bg-[var(--color-chart)] hover:text-[var(--color-bg)] transition-colors shadow-[var(--shadow-subtle)]"
            >
              View All Stocks
            </button>
          </div>
        ...
        {trendLoading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-[140px] rounded-[var(--radius-lg)] bg-[var(--color-border)]/40 animate-pulse"
              />
            ))}
          </div>
        ) : trending.length === 0 ? (
          <div className="card-strong py-12 text-center flex flex-col items-center">
            <FileText
              className="text-[var(--color-text-secondary)] opacity-50 mb-4"
              size={40}
            />
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {trending.map((item, index) => {
              ...
              if (isBullish)
                sigClass = "bg-[var(--color-gain-bg)] text-[var(--color-gain)]";
              if (isBearish)
                sigClass = "bg-[var(--color-loss-bg)] text-[var(--color-loss)]";
              if (isHold) sigClass = "bg-amber-500/10 text-amber-500";

              const hasPrice = item.price !== null && item.price !== undefined;
              const isUp = hasPrice && item.daily_change_pct >= 0;
              ...
              return (
                <div
                  ...
                  className="card cursor-pointer group hover:border-[var(--color-chart)]/40 transition-colors animate-[fadeInY_0.6s_ease-out_forwards] opacity-0 relative z-10 hover:z-20"
                  style={{ animationDelay: `${index * 0.05}s` }}
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-xl font-extrabold font-[Sora] group-hover:text-[var(--color-chart)] transition-colors">
                        {item.symbol}
                      </h3>
                      <p className="text-xs font-semibold text-[var(--color-text-secondary)] mt-1 line-clamp-1 uppercase tracking-widest">
                        {item.name}
                      </p>
                    </div>
                    <div
                      className={`inline-flex items-center px-2 py-0.5 rounded-[var(--radius-sm)] text-[0.7rem] font-bold mono-font ${sigClass}`}
                    >
                      {signal}
                    </div>
                  </div>
                  <div className="flex items-end justify-between mt-6">
                    <div className="mono-font text-2xl font-bold tracking-tight">
                      {hasPrice ? item.price.toFixed(2) : "N/A"}
                    </div>
                    <div
                      className={`mono-font text-sm font-semibold tracking-tight ${isUp ? "text-[var(--color-gain)]" : hasPrice ? "text-[var(--color-loss)]" : "text-[var(--color-text-secondary)]"}`}
                    >
                      {hasPrice
