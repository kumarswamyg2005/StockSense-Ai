...
export default function StockDetail() {
  ...
  if (error || !stock)
    return (
      <div className="card-strong max-w-2xl mx-auto mt-16 text-center shadow-[var(--shadow-hover)]">
        <p className="text-[1.5rem] font-extrabold font-[Sora]">
          Failed to load asset data
        </p>
        <p className="mt-2 text-sm text-[var(--color-text-secondary)]">
          The requested ticker symbol could not be resolved or the server is
          unavailable.
        </p>
        <button
          type="button"
  ...
  return (
    <div
      className={`space-y-8 relative z-10 pt-4 pb-12 max-w-[1400px] mx-auto px-4 md:px-8 transition-opacity duration-300 ${isFetching && !isLoading ? "opacity-50" : "opacity-100 animate-[fadeInY_0.6s_ease-out_forwards]"}`}
    >
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <p className="px-2 py-1 rounded bg-[var(--color-border)] text-[var(--color-text-primary)] text-sm font-bold mono-font">
                {stock.symbol}
              </p>
              <div className="h-4 w-[1px] bg-[var(--color-border)]"></div>
              <p className="text-sm font-medium text-[var(--color-text-secondary)] uppercase tracking-widest">
                Equity
              </p>
            </div>
            <h1 className="text-[clamp(2.5rem,4vw,3.5rem)] font-extrabold font-[Sora] tracking-tight leading-tight">
              {stock.name}
            </h1>
            <div className="mt-4 flex items-baseline gap-3">
              <span className="text-[3rem] font-medium mono-font tracking-tighter leading-none">
                {stock.current_price}
              </span>
              <span className="text-[1.25rem] font-semibold text-[var(--color-text-secondary)] mono-font">
                {stock.currency}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-3">
            <div
              className={`px-6 py-4 rounded-[var(--radius-md)] text-2xl font-medium mono-font tracking-tight ${trendUp ? "bg-gain text-gain" : "bg-loss text-loss"}`}
            >
              {trendUp ? "+" : ""}
              {stock.daily_change}
      ...
      <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="space-y-6">
          ...
          <section className="card border-l-4 border-l-[var(--color-chart)] bg-[var(--color-surface)] text-[var(--color-text-primary)]">
            ...
            <ul className="mt-2 space-y-3">
              {beginnerSummary.map((item, idx) => (
                <li
                  key={idx}
                  className="flex gap-3 text-sm text-[var(--color-text-secondary)] leading-relaxed relative pl-4 before:content-[''] before:absolute before:left-0 before:top-[8px] before:w-1.5 before:h-1.5 before:rounded-full before:bg-[var(--color-chart)]"
                >
                  {item}
                </li>
              ))}
          ...
          <div className="card-strong bg-[var(--color-bg)] border-[var(--color-border)]">
            <h3 className="mb-4 text-sm font-bold uppercase tracking-widest text-[var(--color-text-secondary)]">
              Decision Factors
            </h3>
            <ul className="space-y-3 text-sm text-[var(--color-text-primary)] font-medium leading-relaxed">
              {(
                ...
                <li key={idx} className="flex align-start gap-2">
                  <span className="text-[var(--color-text-secondary)] opacity-50 mt-0.5">
                    •
                  </span>
                  {reason}
                </li>
          ...
          <div className="card bg-[var(--color-surface)] border-[var(--color-border)] text-xs text-[var(--color-text-secondary)] opacity-80 hover:opacity-100 transition-opacity">
            <p className="font-bold uppercase tracking-widest mb-2 font-[Sora]">
              Notice
            </p>
